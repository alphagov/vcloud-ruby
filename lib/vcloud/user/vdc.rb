module VCloud
  ...
  class Vdc < BaseVCloudEntity
    ...
    # @return [Hash{String => VCloud::Reference}] Reference to all Networks in
    #   the vDC, keyed by name.
    def get_network_references_by_name
      Hash[@network_references.collect {|i| [i.name, i]}]
    end

    # Create a vApp from a vApp template.
    # @param [VCloud::InstantiateVAppTemplateParams]
    #   instantiate_vapp_template_params vApp template parameters.
    # @param [VCloud::Client] session Session to create the vApp Template
    #   under.
